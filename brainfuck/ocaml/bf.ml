...
let translate ch =
  String.iter (fun x ->
      match equiv x with "" -> () | s -> Printf.fprintf ch "      %s\n" s)
...
let compile input_file =
  ...
  let temp_file = Filename.temp_file output_file ".c" in
  let command = Filename.quote_command "tcc" [ temp_file; "-o"; output_file ] in
  transpile input_file temp_file;
  (match Sys.command command with
  | 0 -> ()
  | n -> Printf.printf "Error code %d" n);
...
let interpret input_file =
  ...
  and ptr = ref 0
  and i = ref 0 in
  let len = String.length content - 1 in
  let rec aux () =
    if !i >= len then ()
    else (
      incr i;
      ...
      | '-' ->
          ...
          aux ()
      | '.' ->
          arr.(!ptr) |> char_of_int |> print_char;
          aux ()
      | ',' ->
          arr.(!ptr) <- input_char stdin |> int_of_char;
          aux ()
      | '[' ->
          if arr.(!ptr) = 0 then
            let loop = ref 1 in
...
let () =
  let interpreter = ref false and transpiler = ref false in
  try
    for i = 1 to Array.length Sys.argv - 1 do
      ...
      | file ->
          if !interpreter then (
            ...
            interpret file;
            Printf.printf "\n%s - intepreted\n" file)
          else if !transpiler then (
            transpiler := false;
            Filename.remove_extension file ^ ".c" |> transpile file;
            Printf.printf "%s - transpiled\n" file)
          else (
            compile file;
            Printf.printf "%s - compiled\n" file)
    done
  with Help -> ()
